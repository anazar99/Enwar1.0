...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(test_dataloader):
            ...
            kitti_dets, distances, directions = convert_det_to_real_values(detections)
            if len(kitti_dets) > 0:
                kitti_dets[:, 1:] = lidar_to_camera_box(kitti_dets[:, 1:], calib.V2C, calib.R0, calib.P2)
                img_bgr = show_rgb_image_with_boxes(img_bgr, kitti_dets, calib)
                # Print distances and directions
                for i in range(len(kitti_dets)):
                    cls_id = kitti_dets[i, 0]
                    object_type = ''
                    if cls_id == 0:
                    ...
                    print(f"A {object_type} is detected {distances[i]:.2f} meters away at a direction of {directions[i]:.2f} degrees")
