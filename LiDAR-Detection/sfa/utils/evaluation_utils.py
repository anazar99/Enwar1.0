...
def convert_det_to_real_values(detections, num_classes=3, view='front'):
    kitti_dets = []
    distances = []
    directions = []
    ...
    for cls_id in range(num_classes):
        if len(detections[cls_id]) > 0:
            for det in detections[cls_id]:
                ...
                # (scores-0:1, x-1:2, y-2:3, z-3:4, dim-4:7, yaw-7:8)
                _score, _x, _y, _z, _h, _w, _l, _yaw = det
                _yaw = -_yaw  # Invert yaw angle

                # Convert BEV coordinates to real-world coordinates
                ...
                # Centering coordinates for distance and direction calculations
                x_centered = x - (cnf.bound_size_x / 2)
                y_centered = y # Use when only looking at front

                # Calculate distance from the point (cnf.bound_size_x / 2, y)
                distance = np.sqrt(x_centered ** 2 + y_centered ** 2)
                distances.append(distance)

                # Calculate direction angle in degrees
                direction_rad = math.atan(y_centered / x_centered)
    ...
    return np.array(kitti_dets), np.array(distances), np.array(directions)
